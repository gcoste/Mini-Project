...
public class Jeu extends JFrame {
    ...
    public void paint(Graphics g) {
        // On remplit le buffer en noir, ça se répercute sur l'arrière plan (toujours définir la couleur avant de dessiner)
...
/* RETRAVAILLER BOUCLE PRINCIPALE UNIQUEMENT QUAND TOUT LE RESTE FONCTIONNERA
        ...
        //On ajoute le missile si il est tiré
        if (ToucheEspace) {
            Missile M = new Missile(vaisseau.x + vaisseau.l / 2, vaisseau.y, Ecran, "MISSILE");
            Objets.add(M);
        }

        //On rajoute une bombe qui tombe du haut de l'écran
        //3/80 de chance de faire tomber une bombe toutes des 100 ms
        int t = (int) (80 * Math.random());
        if (t <= 2) {
            t = 2;
        }
        //On ajoute aux 3/80 chances 1/2 chances d'être réellement lancé, donc 3/160 chances par 100 ms
        // on va simplifier le jeu en lançant un missile au hasard du haut du frame
        if (temps % t == 0) {
            Bombe B = new Bombe((int) (Math.random() * Ecran.width), 0, Ecran, "BOMBE");
            Objets.add(B);
        }

        //On parcours tous nos objets à la recherche des missiles
        for (int k1 = 0; k1 < Objets.size(); k1++) {
            Objet O1 = (Objet) Objets.get(k1);
            //On trouve un missile
            if ((O1.actif) && (O1.nom_objet == "MISSILE")) {
                for (int k2 = 0; k2 < Objets.size(); k2++) {
                    Objet O2 = (Objet) Objets.get(k2);
                    //On cherche les aliens pour faire des couples miisiles/aliens
                    if ((O2.actif) && (O2.nom_objet == "ALIEN")) {
                        //On regarde si l'alien et le missile sont en collision
                        if (O1.Collision(O2)) {
                            //On vérifie si il n'y a plus d'alien
                            if (nombreAliensVivants > 1) {
                                nombreAliensVivants--;
                            } else {
                                nombreAliensVivants--;
                                finjeu = true;
        ...
        //On recherche maintenant les bombes
        for (int k = 0; k < Objets.size(); k++) {
            Objet O = (Objet) Objets.get(k);
            //On trouve une bombe
            if (O.nom_objet == "BOMBE") {
                //Il y a bien collision entre une bombe et le vaisseau
                if ((O.actif) && (vaisseau.Collision(O))) {
                    //On éteint la bombe, elle disparait
                    O.actif = false;
                    //On vérifie qu'il reste des vies au joueur
                    if (nombreViesRestantes > 0) {
                        nombreViesRestantes--;
                    } else {
                        finjeu = true;
                    }
        ...
        // On vérifie qu'un alien actif ne touche pas le vaisseau
        for (int k1 = 0; k1 < Objets.size(); k1++) {
            Objet O1 = (Objet) Objets.get(k1);
            if ((O1.actif) && (O1.nom_objet == "ALIEN")) {
                if (vaisseau.Collision(O1)) {
                    O1.actif = false;
                    //Si il touche le vaisseau, on perd une vie sauf si l'on en a plus
                    if (nombreViesRestantes > 0) {
                        nombreViesRestantes--;
                    } else {
                        finjeu = true;
            ...
            //Si un alien est en bas, le jeu est terminé
            if ((O1.y >= this.getSize().width) && (O1.nom_objet == "ALIEN")) {
                finjeu = true;
            }
        }
        ...
        if (finjeu) {
            //On désactive tous les objets
            for (int k1 = 0; k1 < Objets.size(); k1++) {
                Objet O1 = (Objet) Objets.get(k1);
                O1.actif = false;
            }
            //On voit si c'est victoire ou défaite
            //On créé ensuite l'objet message correspondant
            //L'objet message sera traité comme un objet par le Paint
            //On a donc peu de choses à modifier
            if (nombreAliensVivants == 0) {
                Objet m = new Message(true, score, Ecran);
                Objets.add(m);
            } else {
                Objet m = new Message(false, score, Ecran);
                Objets.add(m);
            }
        }
